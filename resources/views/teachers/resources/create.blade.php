<x-student>
    ...
    <section id="body">
        <div class="container mt-3">
            <h2>Formulario de creación de recursos</h2>
            <p class="fs-5">Gestiona tus recursos apropiadamente, recuerda subir unicamente los archivos que consideras vitales para tu lección</p>
            {!! Form::open(['route' => ['teacher.lessons.resources.store',$lesson],'method'=>'post','files'=>true]) !!}
            @include('teachers.resources.partials.form')
            {!! Form::submit('Crear recurso', ['class'=>'btn btn-danger fs-5 text-light']) !!}
            {!! Form::close() !!}
        </div>
    </section>
    @push('scripts')
