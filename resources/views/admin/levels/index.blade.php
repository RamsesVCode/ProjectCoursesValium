...
@section('content')
    ...
    <div class="d-flex gap-2 justify-content-end">
        <a class="btn btn-danger text-light mb-3" href="{{route('admin.levels.create')}}"><i class="fas fa-plus"></i> Crear nivel</a>
    </div>
    {{-- LESSONS --}}
        ...
        <table class="table table-hover text-center">
            ...
            @foreach ($levels as $level)
                <tr >
                    ...
                    <td style="min-width:110px;">{{$level->created_at->diffForHumans()}}</td>
                    <td class="text-center">
                        <a href="{{route('admin.levels.edit',$level)}}" class="mr-2 text-success text-decoration-none" title="Editar"><i class="fas fa-pen"></i></a>
                        ...
                        <form action="{{route('admin.levels.destroy',$level)}}" method="post" id="form-{{$loop->iteration}}">
                            @csrf
                            @method('delete')
                        </form>
                    </td>
